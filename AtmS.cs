...
public class AtmS : MonoBehaviour {
	...
	void OnRenderImage(RenderTexture source,RenderTexture destination){
		...
		var p = GL.GetGPUProjectionMatrix(camera.projectionMatrix, !camera.forceIntoRenderTexture);// Unity flips its 'Y' vector depending on if its in VR, Editor view or game view etc... (facepalm)
		p[2, 3] = p[3, 2] = 0.0f;
		p[3, 3] = 1.0f;
